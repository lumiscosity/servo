...
pub trait ThreadSafeLayoutNode<'dom>: Clone + Copy + Debug + NodeInfo + PartialEq + Sized {
    ...
    /// If this node is an embed element, returns its browsing context ID. If this node is
    /// not an embed element, fails. Returns None if there is no nested browsing context.
    fn embed_browsing_context_id(&self) -> Option<BrowsingContextId>;

    /// If this node is an embed element, returns its pipeline ID. If this node is
    /// not an embed element, fails. Returns None if there is no nested browsing context.
